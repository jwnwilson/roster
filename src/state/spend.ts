...
function agentsWithSpend(state: RosterState) {
  return state.agents.flatMap((agent) => {
    ...
    return usage ? [{ agent, costUsd: usage.costUsd, hasIncludedCost: usage.hasIncludedCost ?? false }] : []
  })
}
...
export function selectSpendByProvider(state: RosterState): SpendBar[] {
  ...
  const providers = spendBars(
    [...totals].map(([provider, value]) => ({
      ...
      hasIncludedCost: spending.some(
        (item) => providerOf(state, item.agent.runner) === provider && item.hasIncludedCost,
      ),
    })),
  )
    ...
    return {
      ...
      models: spendBars(
        [...models].map(([model, value]) => ({
          ...
          hasIncludedCost: spending.some(
            (item) =>
              providerOf(state, item.agent.runner) === provider.key &&
              item.agent.model === model &&
              item.hasIncludedCost,
          ),
        })),
      ),
    }
...
export function selectSpendByAgent(state: RosterState): SpendBar[] {
  return spendBars(
    agentsWithSpend(state).map(({ agent, costUsd, hasIncludedCost }) => ({
      key: agent.id,
      label: agent.name,
      value: costUsd,
  ...
  return spendBars(
    Object.entries(state.spendByProject).map(([id, usage]) => {
      ...
      return {
        ...
        hasIncludedCost: usage.hasIncludedCost ?? false,
      }
    }),
  )
}
 ...
 */
export function selectRosterTotals(state: RosterState): { tokens: number; costUsd: number; hasIncludedCost: boolean } {
  return Object.values(state.agentUsage).reduce<{ tokens: number; costUsd: number; hasIncludedCost: boolean }>(
    (sum, usage) => ({
      tokens: sum.tokens + usage.tokens,
      costUsd: sum.costUsd + usage.costUsd,
      hasIncludedCost: sum.hasIncludedCost || usage.hasIncludedCost === true,
    }),
    { tokens: 0, costUsd: 0, hasIncludedCost: false },
  )
}
