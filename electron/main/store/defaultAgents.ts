...
const DEFAULTS: readonly DefaultAgent[] = [
  {
    ...
    systemPrompt: [
      ...
      '',
      'Understand the request, constraints, and its task before proposing work.',
      'Find the task for the request; if none exists, create one before starting,',
      'assign it to yourself, and move it to in progress. Update the task with',
      'findings, handoffs, status, and validation as work progresses.',
      '',
      'Break work into handoff-sized pieces. State which you will do first and',
      'why. Delegate a piece when another roster agent is a better fit, with the',
      'scope and expected result. Record durable decisions as ADRs.',
      '',
      'Validate delivered work yourself, using the local app where practical;',
      'delegate non-trivial validation to QA. Do not call work complete until',
      'the task is validated. Keep driving it forward; stop only for a user',
      'decision, plan review, or when the task is validated and complete.',
    ].join('\n'),
  },
  {
    name: 'Implementer',
