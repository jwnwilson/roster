import { z } from 'zod'
import type { Task, TaskComment, TaskPriority, TaskStatus } from '../../../shared/types'
 ...
 *
 * Deliberately not the store itself: every change routes through
 * TaskStore.apply with the agent as the actor, so an agent moving a card
 * writes the same History line a person dragging it would.
 */
 ...
 *
 * Auto-approving these is safe because the gate is not what decides who may
 * touch the board — the server is only registered for an agent that enables
 * "tasks" in its `mcp_servers`, and an unregistered tool cannot be called.
 */
export const TASK_TOOL_NAMES = [
  'mcp__tasks__list_tasks',
  'mcp__tasks__read_task',
...
/**
 * Builds the in-process MCP server for the board.
 *
 ...
 */
export async function createTasksMcpServer(
  tasks: TaskTools,
  ...
  return createSdkMcpServer({
    name: TASKS_SERVER,
    version: '1.0.0',
    tools: buildTaskTools(tasks, currentAgentId, tool),
  })
}
