...
export function buildPrompt(input: PlanPromptInput): string {
  ...
  return [
    ...
    '',
    'First run `git worktree list --porcelain`. If the exact worktree below',
    'already exists, reuse it. If it does not exist and the branch already',
    'exists, add the worktree from that branch; otherwise create both:',
    '',
    `    git worktree add ${worktree} ${branch}`,
    `    git worktree add ${worktree} -b ${branch}`,
    '',
    'Use only the exact worktree and branch named above. Do not remove or prune',
    'unrelated worktrees.',
    '',
